...
### Added
- Custom search provider wizard — add any search API via `/browser` → "Add custom provider" without editing source code
- `buildCustomProviderImpl()` — wraps user-defined config into a `ProviderImpl`, slots into the existing registry, fallback chain, and `/web-switch` automatically
